...
#define DATA_TO_WRITE               0xA5A5A5A5                  /* Dummy data to be written into the Flash memories */
...
void writePFLASH(uint32 startingAddr, uint32 _WriteData);
...
typedef struct
{
    ...
    void (*writeFlash)(uint32 startingAddr, uint32 _WriteData);
} Function;
...
void writePFLASH(uint32 startingAddr, uint32 _WriteData)
{
    ...
    for(page = 0; page < PFLASH_NUM_PAGE_TO_FLASH; page++)              /* Loop over all the pages                  */
    {
        ...
        {
            g_commandFromPSPR.load2X32bits(pageAddr, _WriteData, _WriteData); /* Load 2 words of 32 bits each */
        }
...
void writeProgramFlash(uint32 _WriteData)
{
    ...
    g_commandFromPSPR.writeFlash(PFLASH_STARTING_ADDRESS, _WriteData);
...
void verifyProgramFlash(uint32 _WriteData)
{
    uint32 page;                                                /* Variable to cycle over all the pages             */
    uint32 offset;                                              /* Variable to cycle over all the words in a page   */
    ...
    for(page = 0; page < PFLASH_NUM_PAGE_TO_FLASH; page++)                          /* Loop over all the pages      */
    {
        ...
        for(offset = 0; offset < PFLASH_PAGE_LENGTH; offset += 0x4)                 /* Loop over the page length    */
        {
            ...
            if(MEM(pageAddr + offset) != _WriteData)
            {
 ...
 * inside.
 */
void writeDataFlash(uint32 _WriteData)
{
    uint32 page;                                                /* Variable to cycle over all the pages             */
    ...
    for(page = 0; page < DFLASH_NUM_PAGE_TO_FLASH; page++)      /* Loop over all the pages                          */
    {
        ...
        IfxFlash_loadPage2X32(pageAddr, _WriteData, _WriteData); /* Load two words of 32 bits each            */
...
void verifyDataFlash(uint32 _WriteData)
{
    uint32 page;                                                /* Variable to cycle over all the pages             */
    uint32 offset;                                              /* Variable to cycle over all the words in a page   */
    uint32 errors = 0;                                          /* Variable to keep record of the errors            */
