...
uint8 TCP_SendDataArr[5] = {0x01, 0x02, 0x03, 0x04, 0x05};
extern struct tcp_pcb *g_echoPcb;
extern uint8 STATUS_FLAG;
...
void Ifx_Lwip_pollReceiveFlags(void)
{
    ...
    //while (ethernetif_tc29x_timerFlags_interrupt())
    {
        ...
        if((STATUS_FLAG & 0x02) == 0x02)
        {
            STATUS_FLAG &= ~(1 << 1);//clear received Flag

            // tcp_write(g_echoPcb, TCP_SendDataArr, strlen(TCP_SendDataArr), 1);
            echoSend(g_echoPcb, g_echoPcb->callback_arg);
        }
    }
}
